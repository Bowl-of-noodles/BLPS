...
@Component
@Named
public class DelFavAdDelegate implements JavaDelegate {
    private static final Logger logger = Logger.getLogger(CreateAdDelegate.class.getName());
    private final AdsService adsService;
    private final JwtUtil jwtUtil;
    private final CustomerService customerService;

    public DelFavAdDelegate(AdsService adsService, JwtUtil jwtUtil, CustomerService customerService) {
    ...
    public void execute(DelegateExecution delegateExecution) {
        try {
            ...
            customerService.delFromFav(customer, ad_id);
            delegateExecution.setVariable("result", "Ad was successfully deleted from favorites");
            logger.log(Level.INFO, "Current activity is " + delegateExecution.getCurrentActivityName());
            logger.log(Level.INFO, "Ad was successfully deleted from favorites");
        } catch (Throwable throwable) {
            throw new BpmnError("del-from-fav-ad-error", throwable.getMessage());
        }
    }
}
